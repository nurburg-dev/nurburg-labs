...
#### Rules for frontmatter of `README.md`

1. There should be a front matter to `README.md` according to the specification document `docs/specification-for-experiments.md`
2. `intent` should be `challenge`
3. `challengeDetails` are required.
4. For `id` in `challengeDetails` in frontmatter - scan `challenge-*/.nurburgdev/README.md` for the max value in front matter. new id should be 1 more that.

#### Structure for challenge `README.md` content/body

1. In content/body of the markdown never use `h1` or `#` heading. Use `##` or `###` or `####` or `#####` or `#####`
2. Body of a challenge should be around 300-500 words.

#### Writing guidelines for `README.md` content/body

Follow the guidelines provided in `README_GUIDELINE_V1.md` folder.
...
Change directory to the challenge directory and run `nd devcontainer --features=<comma-separated-features>`. Replace `<comma-separated-features>` with a list of features inferred from <programming-language> <stack> arguements. allowed list of features are 1. `postgresql` 2. `mysql` 3. `kafka` 4. `typescript` 5. `go` 6. `java` 7. `python` 8. `temporal`
