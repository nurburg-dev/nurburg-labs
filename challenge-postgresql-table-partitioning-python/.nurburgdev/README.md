...
Install dependencies, reset the schema, load dev data, and start the server:

```bash
uv sync   # install dependencies

# drop and recreate transaction_logs from schema.sql
PGPASSWORD=password psql -h postgresdb -U user -d transactiondb -c "DROP TABLE IF EXISTS transaction_logs CASCADE;"
PGPASSWORD=password psql -h postgresdb -U user -d transactiondb -f schema.sql

# stream CSV into the table
wget -O- https://nurburg-dev-fluentbit.s3.eu-central-1.amazonaws.com/0005-transactions.csv | PGPASSWORD=password psql -h postgresdb -U user -d transactiondb -c "\COPY transaction_logs FROM STDIN CSV HEADER"

# start the API on port 8000
uv run uvicorn src.index:app --host 0.0.0.0 --port 8000 --reload
```

To start fresh at any point, repeat the reset and seed steps above. Connect to the database directly with:

```bash
PGPASSWORD=password psql -h postgresdb -U user -d transactiondb
```
...
```bash
curl "http://localhost:8000/transactions?start_date=2023-01-01&end_date=2023-01-31"
```
